...
export class ZohoComponent implements OnInit, AfterViewInit {
    ...
    openConnectModal() {
        ...
        if (!this._featureAuthService.features.integrations['zoho'] && !this._featureAuthService.features.integrations['zoho_limited']) {
            return;
        }
    ...
    connect() {
        ...
        jQuery('#zoho_btn').html('please Wait...').attr('disabled', true);
        let data = {
        ...
        this._integrationService.authorization(data, 'zoho')
            .subscribe((response) => {
                console.log(response);
                if (response.response.hasOwnProperty('error')) {
                    this.isError = true;
                    jQuery('#zoho_btn').html('Connect').attr('disabled', false);

                    this.errorMsg = 'Invalid Credentials - Please re-enter your Zoho-crm Auth Token ';
                    ...
                    if(!this.isCheckingTestAccount){
                        ...
                        jQuery('#zoho_btn').html('Reconfigure').attr('disabled', false);
                        window.toastNotification('You have successfully integrated with zoho.');
                      }else{
    ...
    test() {
        ...
        if(!this.insertFields.find((el)=>{ return el['calc']==='lName'})){
            if(!this.insertFields.find((el)=>{ return el['calc']==='lastName'})){
            this.mapError = 'Warning - Last Name Field is required.';
            this.isMapError = true;
            jQuery('#zoho-test').html('Test Connection').attr('disabled', false);
            return;
            }
        ...
        if (data.map_fields.length > 0) {
            this._integrationService.testSaveLead('zoho', this.jsonBuilderHelper.getJSONBuilt()._id, data)
                .subscribe((response) => {
                    console.log(response);
            ...
            jQuery('.btn-test').html('Test Connection').attr('disabled', false);
            this.isMapError = true;
            this.mapError = 'Minimum 1 field is required for Zoho';
        }
    }
    ...
    getFields() {
        let isIntegrationLimited: boolean = this._featureAuthService.features.integrations['zoho_limited'];
        let isIntegrationFull: boolean = this._featureAuthService.features.integrations['zoho'];
    ...
    getBasicMapFields() {
        ...
        this.mapError = '';
        this._integrationService.getCalcBasicFields('zoho', this.jsonBuilderHelper.getJSONBuilt()._id)
            .subscribe((result) => {
                console.log(result);
    ...
    getFullMapFields() {
        ...
        this._integrationService.getCalcMapFields('zoho', this.jsonBuilderHelper.getJSONBuilt()._id)
            .subscribe((result) => {
                console.log(result);
                if (!result.calc.find((el) => { return el['api_key'] === 'email' })) {
                    this.isMapError = true;
    ...
    sendMapFields() {
        ...
        if (this.isLimited) {
            this._integrationService.sendMapFieldsBasic('zoho', this.jsonBuilderHelper.getJSONBuilt()._id, data)
                .subscribe((result) => {
                    this.notify.emit({
                        action: 'activate'
                    });
            ...
            this._integrationService.sendMapFields('zoho', this.jsonBuilderHelper.getJSONBuilt()._id, this.insertFields)
                .subscribe((result) => {
                    this.notify.emit({
                        action: 'activate'
                    });
    ...
    returnFeatureAccess() {
        let isIntegrartionLimited: boolean = this._featureAuthService.features.integrations['zoho_limited'];
        let isIntegrationFull: boolean = this._featureAuthService.features.integrations['zoho'];
    ...
    premiumPopup(feature: string) {
        this._featureAuthService.setSelectedFeature(feature, 'zoho');
        jQuery('.' + feature).addClass('activegreen limited-label');
        jQuery('#premiumModal').modal('show');
        jQuery('.modal-backdrop').insertAfter('#premiumModal');
    }
    close() {
        jQuery('#zoho-new').modal('hide');
        this.crmAllFileds = [];
        this.calcAllFileds = [];
        this.insertFields = [];
        this.isAccountSelected = false;
    ...
    sync() {
        ...
        this._integrationService.syncCalcLeads('zoho', this.jsonBuilderHelper.getJSONBuilt()._id)
            .subscribe((result) => {
                if (result.status == 1) {
                    jQuery('.sync-leads-zoho').html('Sync');
                    window.toastNotification('Data will be synced with Zoho shortly');
                }
                else {
    ...
    getAccounts(){
        ...
        this.accounts =[];
        this.isAccountSelected = false; 
        this._integrationService.getAccount('zoho')
       ...
       setAccount(key:any,name:any, action:any){
        ...
        this._integrationService.setAccount(data, 'zoho')
          .subscribe((response) => {
