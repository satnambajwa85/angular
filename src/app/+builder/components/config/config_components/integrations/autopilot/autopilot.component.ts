...
export class AutopilotComponent implements OnInit, AfterViewInit {
  ...
  openConnectModal() {
    ...
    if (
      !this._featureAuthService.features.integrations["autopilot"] &&
      !this._featureAuthService.features.integrations["autopilot_limited"]
    ) {
      return;
    }
    ...
    jQuery("#autopilot-new").modal("show");
  }
  openNewConnectModal() {
    this.isAccountSelected = true;
    this.isConnected = false;
  ...
  connect() {
    ...
    jQuery("#btnAutoPilot").html("Please Wait...");
    let data = {
      api_key: this.autoPilotForm.value.apiKey,
      account_name : this.autoPilotForm.value.accountName,
      company: this.company,
      isCheckingTestAccount : this.isCheckingTestAccount
    };

    this._integrationService.authorization(data, "autopilot").subscribe(
      response => {
        if (response!=200 && this.isCheckingTestAccount ){
          this.isError=true;
          jQuery('#btnAutoPilot').html('Connect');
          this.errorMsg='Seems to be your account is expired or API key is not valid.'
        }
        if (response === 400) {
          this.isError = true;
          jQuery('#btnAutoPilot').html('Connect');
          this.errorMsg = 'No autopilotapikey header provided';
        }
        if (response === 401) {
          this.isError = true;
          jQuery('#btnAutoPilot').html('Connect');
          this.errorMsg =  "Invalid Credentials - Please re-enter your Autopilot Api key";

        }if (response === 429) {
          this.isError = true;
          jQuery('#btnAutoPilot').html('Connect');
          this.errorMsg = 'Your request was throttled. Please see \"Rate Limit\" section of documentation at http://developers.autopilothq.com.';
        }if (response === 500) {
          this.isError = true;
          jQuery('#btnAutoPilot').html('Connect');
          ...
          window.toastNotification(
            "You have successfully integrated with AutoPilot"
          );
          this.getList();
          ...
          jQuery('#'+this.autoPilotForm.value.apiKey).removeClass('hide');
        }
      ...
      error => {
        ...
        if (error.error.code === "ENOTFOUND") {
          this.errorMsg =
            "Invalid Credentials - Please re-enter your AutoPilot Api key";
    ...
    this._integrationService
      ...
      .subscribe(
        result => {
          ...
          if (checked) {
            toastText =
              "Integration Activated !! The leads from this calculator will be sent to AutoPilot";
          } else {
            toastText =
              "Integration Deactivated !! The leads from this calculator will not be sent to AutoPilot";
          }
  ...
  getFields() {
    ...
    let isIntegrationLimited: Boolean = this._featureAuthService.features
      .integrations["autopilot_limited"];
  ...
  getFullMapFields() {
    ...
    this.isLeadGenError = false;
    this._integrationService
      .getCalcMapFields("autopilot", this.jsonBuilderHelper.getJSONBuilt()._id)
      .subscribe(
        result => {
          if (
            !result.calc.find(el => {
  ...
  premiumPopup(feature: string) {
    this._featureAuthService.setSelectedFeature(feature, "autopilot");
  ...
  close() {
    ...
    jQuery("#autopilot-new").modal("hide");
    this.crmAllFileds = [];
    this.calcAllFileds = [];
    this.insertFields = [];
    this.thirdstage = false;
  ...
  sync() {
    ...
    this._integrationService
      .syncCalcLeads("autopilot", this.jsonBuilderHelper.getJSONBuilt()._id)
      .subscribe(
        result => {
          if (result.status == 1) {
            jQuery(".sync-leads-mc").html("Sync");
            window.toastNotification(
              "Data will be synced with autopilot shortly"
            );
          } else {
  ...
  addIsfocusedKey() {
    jQuery("#autoPilot_apiKey").addClass("is-focused");
    this.isError = false;
    this.errorMsg = "";
  }

  removeIsfocusedKey() {
    jQuery("#autoPilot_apiKey").removeClass("is-focused");
  }

  getList() {
    this.isLoading = true;
    this.selectedListId = this.config.list_id;
    this._integrationService.getList("autopilot").subscribe(
      response => {
        ...
        if (response.total_items !== 0) {
          ...
          for (let key in lists) {
            if (lists.hasOwnProperty(key)) {
              let obj = {};
              obj["name"] = lists[key].title;
              obj["id"] = lists[key].list_id;
          ...
          this.isLoading = false;
          this.isError=true
          this.errorMsg="Your Account  has been expired or currently not working .Please Reconfigure "
        }
      },
      error => {
  ...
  saveList() {
    if (!this.selectedListId || this.selectedListId === "0") {
      ...
      jQuery("#btnAutoPilotList").html("Please wait...");
      let data = {
        list_id: this.selectedListId,
      ...
      this._integrationService
        .saveList("autopilot", this.jsonBuilderHelper.getJSONBuilt()._id, data)
        .subscribe(
          (response: any) => {
  ...
  getAccounts() {
    ...
    this.isError=false;
    this.errorMsg=""
    this._integrationService.getAccount('autopilot')
  ...
  testAccount(account: any) {
    this.autoPilotForm.value.apiKey = account;
  ...
  setAccount(key: any, action: any) {
    ...
    this._integrationService.setAccount(data, 'autopilot')
      .subscribe((response) => {
        if (action === 'edit') {
