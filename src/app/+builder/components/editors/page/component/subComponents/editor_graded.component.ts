...
@Component({
  selector: 'editor_graded',
  templateUrl: './assets/html/editor_graded.component.html',
  encapsulation: ViewEncapsulation.None
})

export class EditorGraded implements OnInit, OnChanges, OnDestroy {
  ...
  emitterHandler() {
    this.froalaService.getEmitter().subscribe(change => {
      ...
      if (change.event === 'blur') {
        change.data.id === 'graded' + this.editorControl.cta_shares._id + 'share-title' && this.onWysiwygBlur('share-title');
        change.data.id === 'graded' + this.editorControl.cta_shares._id + 'share-desc' && this.onWysiwygBlur('share-desc');
      }
    });
  }
  ...
  initWysiwyg() {
    ...
    setTimeout(() => {
      ...
      this.froalaCtaSocialTitle.options = this.froalaService.getOptions({ handler: this.froalaCtaSocialTitle, includeScores: true, isOnlyAddVariable: true, id: 'graded' + this.editorControl.cta_shares._id + 'share-title' });
      this.froalaCtaSocialDesc.options = this.froalaService.getOptions({ handler: this.froalaCtaSocialDesc, includeScores: true, isOnlyAddVariable: true, id: 'graded' + this.editorControl.cta_shares._id + 'share-desc' });
  ...
  toggleGradedTab() {
    ...
    this.jsonBuilderHandler.getJSONBuilt().GQans = false;
  }
